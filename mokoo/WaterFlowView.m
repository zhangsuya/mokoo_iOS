...
//    }
    ...
    for (UITableView *tabelview in tableviews) {
        tabelview.backgroundColor = viewBgColor;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (cell == nil) {
        ...
        cellSub.backgroundColor = viewBgColor;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    IndexPath *_indextPath = [IndexPath initWithRow:indexPath.row  withColumn:tableView.tag - TABLEVIEWTAG];
//    [self setNeedsLayout];
    if ([_waterFlowViewDelegate respondsToSelector:@selector(waterFlowView:didSelectRowAtIndexPath:)]) {
        [self.waterFlowViewDelegate waterFlowView:self didSelectRowAtIndexPath:_indextPath];
    }
    //i do not know why 这样可以避免模特卡的返回偏移
//    [self performSelector:@selector(setNeedsLayout) withObject:self afterDelay:1];
}
