...
- (id)initWithTitle:(NSString *)title 
{
    if ((self = [super init]))
    {
        ...
        self.titleShadowColor = tsPopColor;
        ...
        popoverController.popoverBaseColor = tsPopColor;
...
//        UIImage *image = [self buttonImage:color borderWidth:borderWidth borderColor:borderColor];
        ...
        button.tag = i++;
        if (i<[buttonsMutableArray count] +1) {
            UILabel *lineLayer = [[UILabel alloc]init];
            lineLayer.frame = CGRectMake(BORDER, buttonY +buttonHeight +BORDER/2, self.bounds.size.width-BORDER*2, 0.5);
            //        [lineLayer setBackgroundColor:[UIColor colorWithRed:107 green:107 blue:107 alpha:0]];
            [lineLayer setBackgroundColor:[UIColor whiteColor]];
...
-(UIImage*)buttonImage:(UIColor*)color borderWidth:(NSUInteger)borderWidth borderColor:(UIColor*)borderColor
{
    ...
    if(borderWidth >0)
    {
        if(!borderColor) borderColor = tsPopColor;
