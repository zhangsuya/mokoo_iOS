...
//    view.backgroundColor = _topBorder.backgroundColor;
    view.backgroundColor = lineSystemColor;
    [self addSubview:view];
...
//        _bottomBorder.frame = CGRectMake(0, self.fs_height, self.fs_width, 1);
        _bottomBorder.frame = CGRectMake(0, self.fs_height, self.fs_width, 0.5);
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([_delegate respondsToSelector:@selector(passCalendarCell:)]) {
        [_delegate passCalendarCell:cell];
    }
...
- (NSInteger)currentSection
{
    switch (_scope) {
        ...
        case FSCalendarScopeWeek: {
            ...
            //修改
            return [_currentPage fs_weeksFrom:self.selectedDate];
        }
        default: {
            break;
        }
...
- (void)performScopeTransitionFromScope:(FSCalendarScope)fromScope toScope:(FSCalendarScope)toScope animated:(BOOL)animated
{
    ...
    if (weekToMonth) {
        ...
        _currentPage = self.selectedDate ?: _today;
    }
...
//                _bottomBorder.frame = CGRectMake(0, size.height, self.fs_width, 1);
                _bottomBorder.frame = CGRectMake(0, size.height, self.fs_width, 0.5);
