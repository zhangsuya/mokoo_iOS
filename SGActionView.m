...
+ (SGActionView *)initNormal
{
    SGActionView *actionView = nil;
...
- (void)setMenu:(UIView *)menu animation:(BOOL)animated{
    ...
    topMenu.frame = (CGRect){CGPointMake(16,90), CGSizeMake(CGRectGetWidth(topMenu.bounds) -16, CGRectGetHeight(topMenu.bounds))};
...
- (void)showTextFieldWithselectedDict:(NSDictionary *)dict selectedHandle:(SGChooseFieldActionHandler)handler
{
    SGTextFieldMenu *menu = [[SGTextFieldMenu alloc]initTextFieldViewWithDict:dict];
    SGActionView *tView = [SGActionView initNormal];
    [menu triggerSelectedAction:^(NSDictionary *index)
     {
         [tView dismissMenu:menu Animated:YES];
         if (handler) {
             handler(index);
         }
     }
     ];
    [tView setMenu:menu animation:YES];
}
+ (void)showChooseMenuWithTitle:(NSArray *)titile
                     itemTitles:(NSArray *)itemTitles
                       limitNum:(NSInteger)num
                 selectedHandle:(SGChooseMenuActionHandler)handler
{
    SGChooseMenu *menu = [[SGChooseMenu alloc] initWithTitle:titile itemTitles:itemTitles limitedNum:num];
    [menu triggerSelectedAction:^(NSArray *selectedArray)
     {
         [[SGActionView sharedActionView] dismissMenu:menu Animated:YES];
         if (handler) {
             handler(selectedArray);
         }
     }
     ];
    [[SGActionView sharedActionView] setMenu:menu animation:YES];
...
+(void)showThreeTextFieldWithTitle:(NSString *)titile selectedHandle:(SGChooseStringActionHandler)handler
{
    SGThreeTextField *menu = [[SGThreeTextField alloc] initTextFieldViewWithString:titile];
    [menu triggerSelectedAction:^(NSString *selectedString) {
        [[SGActionView sharedActionView] dismissMenu:menu Animated:YES];
        if (handler) {
            handler(selectedString);
        }
    }];
    [[SGActionView sharedActionView] setMenu:menu animation:YES];
